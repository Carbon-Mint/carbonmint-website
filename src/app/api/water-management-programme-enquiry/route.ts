...
// Interface for the water management programme enquiry form data
interface WaterManagementProgrammeEnquiry {
  fpcName: string;
  ...
  paddyAcres: string;
  transplantedPaddyAcres: string;
  averageLandHolding: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['fpcName', 'registrationNumber', 'state', 'totalMemberFarmers', 'totalLandAcres', 'paddyAcres', 'transplantedPaddyAcres', 'contactPersonName', 'designation', 'mobileNumber', 'consent'];
    ...
    // Validate that paddy acres don't exceed total land acres
    const totalLand = parseFloat(data.totalLandAcres);
    const paddyLand = parseFloat(data.paddyAcres);
    const transplantedLand = parseFloat(data.transplantedPaddyAcres);

    if (isNaN(totalLand) || isNaN(paddyLand) || isNaN(transplantedLand)) {
      return NextResponse.json(
        { error: 'Invalid land acreage values. Please enter valid numbers.' },
        { status: 400 }
      );
    }

    if (paddyLand > totalLand) {
      return NextResponse.json(
        { error: 'Paddy acres cannot exceed total land acres.' },
        { status: 400 }
      );
    }

    if (transplantedLand > paddyLand) {
      return NextResponse.json(
        { error: 'Transplanted paddy acres cannot exceed total paddy acres.' },
        { status: 400 }
      );
    }

    const database = await getDatabase();
    const collection = database!.collection<WaterManagementProgrammeEnquiry>('carbonmint_com_water_management_programme_enquiries');

    // Store water management programme enquiry
    const enquiry: WaterManagementProgrammeEnquiry = {
      ...
      paddyAcres: data.paddyAcres.trim(),
      transplantedPaddyAcres: data.transplantedPaddyAcres.trim(),
      averageLandHolding: data.averageLandHolding.trim(),
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Water management programme enquiry submitted successfully',
      enquiryId: result.insertedId 
    }, { status: 200 });

  } catch (error) {
    console.error('Water management programme enquiry submission error:', error);
    return NextResponse.json({ 
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching water management programme enquiries:', error);
    return NextResponse.json({ 
      error: 'Internal Server Error' 
    }, { status: 500 });
  }
